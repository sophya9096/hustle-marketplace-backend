...
exports.getAllBuyers = asyncHandler(async (req, res) => {
  ...
  const buyerObj = allRoles.find((roleObj) => roleObj.role === "buyer");

  const allBuyers = await User.find({ roleID: buyerObj._id });
  console.log(allBuyers); */

  const allBuyers = await User.find({ roleID: "633edda216a1dad303f9f676" });
  // console.log(allBuyers);

  allBuyers &&
    res.status(200).json({ status: "success", data: { buyers: allBuyers } });
});
...
exports.getAllSellers = asyncHandler(async (req, res) => {
  ...
  const allSellers = await User.find({ roleID: buyerObj._id });
  console.log(allSellers); */

  const allSellers = await User.find({ roleID: "633eddaa16a1dad303f9f67b" });
  // console.log(allSellers);

  allSellers &&
    res.status(200).json({ status: "success", data: { sellers: allSellers } });
});
